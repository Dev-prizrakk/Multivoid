...
| `coop/player/` | remote players and per-player state | `remote_player` + `remote_player_ragdoll`, `puppet_drive`, `puppet_carry_drive`, `puppet_body_yaw`, `puppet_footsteps` (driving the engine pawn), `players_registry`, `player_profile_store` (the host's copy of every player's profile: held in memory, cut to disk with the world save), `roster` + `roster_ledger` (who is in each slot), `nickname_arbiter`, `nick_color`, `nameplate`, `hand_item`, `local_body`, `local_streams`, `client_model` + `skin_registry` + `skin_preview` + `skin_effects` (skins), `movement_ledger`, `player_damage`, `death_revive` + `run_end_travel` (a trip to the menu the player did not ask for is refused and answered with a revive), `ragdoll_gate`, `sleep_sync`, `item_activate`, `flashlight_click_sound` |
...
| `ue_wrap/engine/` | engine objects | `engine` (world, actors, spawning), `engine_pawn`, `engine_component`, `engine_widget` + `umg_build` (UMG), `engine_bones`, `engine_attach`, `engine_audio`, `engine_nav`, `engine_save` + `save_capture` + `save_to_slot_hook` (the one detour on the engine's save function: a gate before a world save, a notice after) + `save_browser` + `gvas_meta` (save files), `engine_mainplayer`, `engine_playerragdoll`, `spawn_gate`, `spawn_menu`, `scs_rig`, `world_identity` (which world is current) |
| `ue_wrap/actors/` | the game's actor classes | `puppet` + `puppet_spawn`, `prop`, `kerfur`, `wisp`, `inventory`, `vitals`, `sleep`, `sleeping_bag`, `swinger`, `save_record` (the Fstruct_save codec, and the actor's own getData/loadData), `floppy_disc`, `begin_equipment`, `broom` |
| `ue_wrap/devices/` | keyed devices | `door`, `door_box`, `lightswitch`, `passwordlock`, `power_control`, `garage`, `cargo_lift`, `appliance`, `base_window`, `grime`, `windturbine`, `drone`, `atv` + `atv_condition`, `laptop`, `portable_pc`, `floppybox`, `serverbox` |
