...
`cmd.exe`, so a pipeline with `xargs` exited 255 and was recorded verified (`8a96761d`: `bash -lc`);
an anchor that BEGINS with a command but also contains a `path:line` was classed by the path and
never re-run (`8a96761d`: command-first); `tail` was not a legal command anchor, so a real anchor was
classed as a quote (`a10af029`: the read-only text tools added). What the pass found in the DESIGN it
