...
- **CENSUS THE ENTRY, NOT THE VERB — an `EX_LocalVirtualFunction` verb is invisible, but the engine-dispatched ENTRY one call above it usually is not.** `[V]` 2026-09-04 on `AserverBox_C`: `fix`, `breakServer`, `check`, `insertFloppy`, `ejectFloppy` and `launchServerMinigame` ALL resolve `EX_LocalVirtualFunction`, and on that table the honest verdict is "no seam, needs the unbuilt bytecode gate". It was wrong for THREE of the four moments the design needed: `ejectFloppy` is reached from `actionOptionIndex` (`@4519 if(action==Use(4))` -> `@4560 if(lookatFloppyButton)` -> `@4574`), a UFunction this project ALREADY intercepts; `insertFloppy` from the `Box` component's `ComponentBeginOverlap` delegate, which hands over the disc as an argument; the transformer's completion from a `turnedOn` delegate. Only the server `fix` genuinely lacks a visible entry. UE makes the mistake systematic: player-facing events are dispatched by the ENGINE (so they cross ProcessEvent) while everything the graph then calls on ITSELF is local — the invisible verbs sit one call BELOW a visible boundary almost by construction. **Look here FIRST:** before writing "no seam exists", walk the call graph UPWARD (`bp_reflect.py` + resolve `StackNode`/`VirtualFunctionName` on every caller, or `bp_cfg` for which event's offset reaches the branch); only when every hop to the top is local is the lane really blocked. [[lesson-census-the-entry-not-the-verb]]
