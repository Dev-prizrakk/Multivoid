...
- **AN EXCLUSION APPLIED BEFORE THE ORDERING CANNOT BE FIXED BY RE-ORDERING** (2026-09-03: an amortised
  sweep dropped `touched` docs BEFORE sorting, and the skill's own steps touch `MEMORY.md`,
  `LESSONS.md` and `CLAUDE.md` on EVERY close — so the three docs the reading order lists FIRST were
  excluded from every queue forever. They hold **194 label rows**; the one real close surfaced **17**,
  and `grep -o '"CLAUDE.md"' docs_census_state.json` = **0**, never stamped once. Two rounds went into
  better ORDERINGS — all correct, all downstream of a set the filter had already emptied — before a
  critic asked which LINE brings their turn. The most-read docs are the most-touched, so a
  touched-exclusion guarantees they are the least swept). *Look FIRST:* when a queue never yields some
  member, read the pipeline IN ORDER and find the first stage that can remove it; a ranking, a budget
  and a cap only redistribute what the filter admitted.
  `memory/lesson-an-exclusion-applied-before-the-ordering-cannot-be-fixed-by-reordering.md`
- **ORDER A LOG BY LINE NUMBER WHEN THE EVENTS SHARE A TIMESTAMP -- a one-second stamp cannot order events inside that second, and it fails precisely when you are looking at a race.** `[V]` 2026-09-04, on a player's 56,270-line field log: reading grep hits in return order told a coherent story -- `OnSpawn` -> `GRAB-IN` -> `OnDestroy` -> a pose arriving for a destroyed actor, i.e. a destroy-then-pose leak. It was reported to the user in those terms. `[V]` `grep -n` gives the real order: the unresolved pose is line **47367** and the `OnSpawn` is **47368** -- the pose arrived BEFORE the spawn, an unreliable packet overtaking its own reliable broadcast, healing a millisecond later. The reversal changed the POPULATION, not just the sentence: the 205 warnings then split into 114 across 82 keys at 1-8 packets each (the race) and **91 on ONE key with `eid=0` and a zero `OnSpawn` count** (a real sustained gap -- a peer holding an item the receiver never got). One log line, two bugs, and the wrong order hid the discriminator entirely. Non-obvious because a timestamp LOOKS like an ordering key, grep output is usually chronological because it usually is, and the wrong order told a story that explained the reported symptom. **Look here FIRST:** before drawing any causal arrow inside one log second, use `grep -n` and read LINE NUMBERS; if the stamp's resolution is coarser than the gap you are reasoning about, only file order can answer. And when one repeated log line has a wide count distribution, the tail and the body are usually different bugs -- find the discriminator before treating them as one. `memory/lesson-order-a-log-by-line-number-when-events-share-a-timestamp.md`
...
- **A DETECTOR CAN RUN FOREVER ON A FORM THE CORPUS NEVER WRITES — count what it MATCHED, not what it passed; a gate with zero inputs is PASS and "everything checked" is PASS.** `[V]` 2026-09-03: `lessons_gate`'s check A2 (`check_quoted_cites`, written 2026-08-30 after an extraction moved five cited facts and the positional gate passed all five) fires only on `` `file:line` says "…" `` — a form occurring **5 times in the whole 1,613-doc read set and 0 times in `docs/LESSONS.md`, the ledger it guards**, against 255 plain `path:line` citations in that one file. It is not dead in the caller sense (it IS called every run); it is called, correct, and matching nothing, which no caller census finds — and with no inputs it also never got to be shown wrong: its first real hit was a FALSE positive (a quote wrapping across two comment lines, where even the joined text read "this // module"). The paying question is the inverse: what form does the corpus ACTUALLY use? Here, a citation beside a backticked symbol — 1,816 of 5,302. **Look here FIRST:** `len(TRIGGER_RE.findall(corpus))` before trusting any long-green gate; then `status_grammar.py` `Resolver.content` and drill arm A4 for the two-strength rebuild. [[lesson-a-detector-can-run-forever-on-a-form-the-corpus-never-writes]]
- **A FIX WRITTEN UNDER PRESSURE CLOSES ONE DIRECTION OF THE RULE IT DISCOVERS — the instance is what you can test, so the instance is what you fix; the RULE arrives one round later.** `[V]` 2026-09-03, four consecutive rounds of one /qf DIFF pass, each finding the PREVIOUS round's fix incomplete the same way: a scope discriminator that asked the grammar instead of the file; a seventh verdict token that covered one rung of four; `NOT A LABEL` attributed on the REJECTION side while a `drift` row could still be verdicted STALE DONE straight into D8's falsifier input; a content pin over the paths the close NAMES while the history commit is `git add -A`; an exemption window reaching forward to the line introducing a quote but not back to the line carrying it. Knowing the pattern did not prevent round 3 from shipping a one-sided gate — only round 4's question did. **Look here FIRST:** after fixing what a review named, ask its three questions in order — DIRECTION (is the mirror case closed?), SCOPE (is that the set the property is about, or one I derived?), INSTRUMENT (the rule names one producer; how many does the data have?). And read the block you remove instead of slicing it by index: retiring that token also deleted the adjacent shipped `contra` refusal. [[lesson-a-fix-written-under-pressure-closes-one-direction-of-the-rule-it-discovers]]
- **A DRILL THAT ASSERTS A GATE'S VALUE IN ITS GREEN STATE PASSES A GATE THAT WAS NEVER WIRED — `== 0` is satisfied by a field nothing ever computes.** `[V]` 2026-09-03: `memref-dead` shipped as a ratchet at target 0 while its compute block had silently failed to apply (a `t.replace()` whose anchor did not match, replacement count unchecked), so `dead_refs()` saw 3 dead pointers and `ratchet_values()` reported 0 — and the drill arm PASSED, because it exercised the DETECTOR in its red state and the RATCHET only in its green one. Same shape twice more within hours: a new verdict token drilled with `TOKEN in VERDICTS` (the vocabulary grew; nothing showed the refusal binding), and `ro-lost` declared GATED while the trailer wrote a literal 0. **Look here FIRST:** for any gate, ratchet or counter ask *which assertion goes red if the WIRE between detector and number is cut* — if none, the arm tests a detector. Show the number NON-ZERO with the defect present, and drive the whole path end to end comparing the OUTPUT to the declared vocabulary (that second form caught a second column, `running-totals`, declared since the schema was written and emitted by no close in its life). [[lesson-a-drill-that-asserts-a-gates-value-in-its-green-state-passes-a-gate-that-was-never-wired]]
...
- **A RADIUS BOUNDED IN DOCS IS NOT A HAND CHECK BOUNDED IN WORK -- bound the unit the WORK is paid in, and print both numbers.** `[V]` 2026-09-03, the FIRST real `/documentize` close: the census bounded its radius to 67 DOCS and handed the hand **714 rows**, against a design that said "tens, not thousands" -- because a doc's status-line count is unbounded (`docs/LESSONS.md` 85, `CLAUDE.md` 71, one research finding 63), so editing ONE line of a doc owed a verdict on every status line in it. Fixed by measurement: a TOUCHED doc contributes only rows whose line-hash is absent from its baseline (714 -> 119), and the amortised sweep spends its OWN row budget instead of what is left of a shared one -- shared, the session's 119 rows ate it and left 1 swept doc of 1,521 candidates (~1,521 closes to cross the tree); separate, ~127. A diff-scoped doc must then NOT be stamped "censused", or it reaches the back of the queue having had only its new lines read. **Look here FIRST:** the drills could not see this -- their scratch repos hold two-line docs, where docs and rows are the same order of magnitude -- so for any bounded-work mechanism print the bounded unit AND the paid unit every run, and give an amortised queue a budget the foreground cannot spend. [[lesson-a-radius-bounded-in-docs-is-not-a-hand-check-bounded-in-work]]
...
- **A CHECK WIRED INTO CI MUST FIRST BE RUN IN CI'S ENVIRONMENT.** 2026-09-04, three times in one
  session, three red boards -- and every one was the RUNNER, on tooling that was fine. A drill's
  premise asserts every cite root is present; CI has no `research/`, so the premise failed and took
  six arms with it. A gate scanned `*_drill.py`; excluding them then HID a real leak in one. A drill
  arm asserted "with the real corpus it is a number again"; CI has no corpus. Each time I had
  "tested the CI path" by simulating ONE missing piece while the runner is missing several: no
  `CLAUDE.md`, no `memory/`, no `research/`, no `docs/security/`, no submodules. *Look FIRST:*
  enumerate what the runner lacks FROM THE WORKFLOW FILE, decide per arm whether it needs any of it,
  and make those SKIP with a stated reason rather than fail -- the distinction
...
- **A SUPPRESSION'S KEY DECIDES ITS BLAST RADIUS, not the instance you were looking at.** 2026-09-04.
  ...
  GENERIC form, so that one entry cleared the pattern in **every file in the repo**, including the doc
  S3 had been written for hours earlier. `[V]` the founding incident's exact prose read as already
  acknowledged; every arm passed; the gate printed green; the signal was dead. **The better the
  pattern, the wider the waiver** -- and nothing distinguishes "green because clean" from "green
  because silenced". *Look FIRST:* for any ack / allowlist / waiver / noqa / expected-failure, ask
  what the KEY is and MEASURE what else matches it; carry the LOCATION in the key, not just the
  content; and make a drill's fixtures SYNTHETIC, or their waiver covers the real thing they imitate.
  A suppression is a claim about a SET -- write the set down first.
  `memory/lesson-a-suppressions-key-decides-its-blast-radius.md`
...
- **CENSUS THE ENTRY, NOT THE VERB — but the entry's dispatch is a MEASUREMENT, not a category.** `[V]` 2026-09-04 on `AserverBox_C`: `fix`, `breakServer`, `check`, `insertFloppy`, `ejectFloppy` and `launchServerMinigame` ALL resolve `EX_LocalVirtualFunction`, and on that table alone the honest verdict is "no seam". Walking UPWARD is right and found two real seams the verb table hid — `insertFloppy` sits under the `Box` component's `ComponentBeginOverlap` `BndEvt__` handler (PE-visible, cancellable, disc as an argument), and `fix` ENDS in `EX_CallMulticastDelegate(fixed__DelegateSignature)`, whose bound handlers dispatch through ProcessEvent. **But the same pass asserted a third seam it never measured, and it was false:** `ejectFloppy` is reached from `actionOptionIndex`, which is NOT engine-dispatched — `[V]` `mainPlayer::useSelectedAction` calls it as `EX_Context{EX_LocalVirtualFunction('actionOptionIndex')}` through an interface (one call site for every interactable class), PE- and Func-invisible, corroborated independently at `coingun_collect.cpp:14-17`; and the interceptor cited as proof we "already hook it" had been RETIRED as never-firing dead code three months earlier (`kerfur_convert.cpp:95-101`). **The rule that survives:** a DELEGATE on the path is a real tier-1 seam; a player-facing VERB NAME is not evidence of engine dispatch — resolve the caller's opcode. And when both fail, tier 3 (`vm_dispatch`, the shipped `0x45` swap) sees every `EX_LocalVirtualFunction` BY NAME with the target as Context, which is what the whole table above actually resolves to. **Look here FIRST:** walk upward with `bp_reflect.py`, resolve `StackNode`/`VirtualFunctionName` on every caller INCLUDING the out-of-class one, and check the verb's TAIL for a multicast broadcast before concluding anything. [[lesson-census-the-entry-not-the-verb]]
- **A DELEGATE BROADCAST IS A SEAM ONLY IF SOMETHING BINDS IT — `EX_CallMulticastDelegate` is a dispatch opcode, not evidence of dispatch.** `[V]` 2026-09-04: hunting a visible completion seam for a client's server fix, every verb on `AserverBox_C` resolved `EX_LocalVirtualFunction` (invisible to both seams) and then `fix()` turned out to END in `EX_CallMulticastDelegate(fixed__DelegateSignature)` (`serverBox.cpp:1266-1288`) — which reads as a ProcessEvent seam and was written into the arc as one. It reaches NOBODY: `[V]` across the 306 kismet dumps in `research/bp_reflection/`, `grep -l fixed` returns exactly three files (`serverBox` itself, `mainPlayer`'s unrelated `fixCrouch`, flavour text in `_map_untitled_1`), so the invocation list is empty and no `ProcessEvent` call happens. The SIBLING delegate on the same class proves the distinction rather than the rule — `[V]` `serverBroke` IS bound by `ui_console` (the SAT terminal, 9 mentions + 3 delegate binds). An unbound delegate is normal (authors broadcast defensively), warns nowhere, and looks identical in bytecode to a bound one; worse, the broadcast is usually the verb's LAST statement, so the trap sits exactly where you look when you want a completion seam. **Look here FIRST:** census the BINDERS, not the broadcast — `grep -l <DelegateName> research/bp_reflection/*.json` over the whole corpus, state the denominator, and look for `BndEvt__*` functions or `EX_AddMulticastDelegate` in the candidate subscribers. A broadcast proves a sender; only a binder proves a seam. `memory/lesson-a-delegate-broadcast-is-a-seam-only-if-something-binds-it.md`
