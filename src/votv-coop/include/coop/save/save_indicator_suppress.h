...
// forward to the original. A live run caught NEITHER firing while the indicator showed, so the
// painter is a third path or the Func patch misses these two, and until that is settled there
// is nothing to no-op. saveObjects, the disk write and the transfer are never touched.
#pragma once
...
// land inside this synchronous window, or within ~3 s of its close, are reported. Both are
// game thread only, as every entry point here is.
void Begin();
void End();
